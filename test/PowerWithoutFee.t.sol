...
contract PowerWithoutFee is BaseSetup {
    ...
    function testDistinvestProof() public {
        ...
        S_ProofState memory _proof = market.getProofState(normalproof);
        assertEq(_proof.state.amount0(), 300000000000, "before disinvest:proof value error");
        assertEq(_proof.state.amount1(), 100000000000, "before disinvest:proof value error");
        assertEq(_proof.invest.amount1(), 300000000000, "before disinvest:proof quantity error");
